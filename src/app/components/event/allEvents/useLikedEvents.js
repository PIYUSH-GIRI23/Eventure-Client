...
import { getLikedEventsAction, rateEventAction, derateEventAction, bookmarkEventAction, debookmarkEventAction } from "@/app/actions/eventAction"
...
export const useLikedEvents = () => {
    ...
    const fetchLikedEvents = useCallback(async () => {
        ...
        try {
            ...
            const response = await getLikedEventsAction(payload)
            if (!response.success) {
                if (response.statusCode === 401 || response.statusCode === 403) {
                    performLogout()
                    return
                }
                setError(response.message || "Failed to fetch liked events")
                return
            }
            ...
            setError(err.message || "An error occurred while fetching liked events")
        } finally {
            setLoading(false)
        }
    ...
    }, [isClient, isLoggedIn, fetchLikedEvents])
