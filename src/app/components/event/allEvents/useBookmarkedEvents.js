...
import { getBookmarkedEventsAction, rateEventAction, derateEventAction, bookmarkEventAction, debookmarkEventAction } from "@/app/actions/eventAction"
import { updateEventBookmarked, forceRefresh } from "@/app/state/slices/eventsSlice"
...
export const useBookmarkedEvents = () => {
    ...
    const fetchBookmarkedEvents = useCallback(async () => {
        ...
        try {
            ...
            const response = await getBookmarkedEventsAction(payload)
            if (!response.success) {
                if (response.statusCode === 401 || response.statusCode === 403) {
                    performLogout()
                    return
                }
                setError(response.message || "Failed to fetch bookmarked events")
                return
            }
            ...
            setBookmarkedEvents(response.data.events || [])
        } catch (err) {
            setError(err.message || "An error occurred while fetching bookmarked events")
        } finally {
            setLoading(false)
        }
    }, [isClient, isLoggedIn, type])
    ...
    }, [isClient, isLoggedIn, fetchBookmarkedEvents])
